...
function verOfertaslimitadas($inicio, $regPag){//cambiar a getOfertas
    $conex=Conexion::getInstance();
    $sql="SELECT *, DATE_FORMAT(fechaFin, '%d/%m/%Y') AS fechaF, DATE_FORMAT(fecha_creacion, '%d/%m/%Y') AS fechaC FROM ofertas LIMIT $inicio, $regPag";
    // Especificamos el fetch mode antes de llamar a fetch()
    $stmt = $conex->dbh->prepare($sql);
    $stmt->setFetchMode(PDO::FETCH_ASSOC);
    // Excecute
...
function getOferta($id){
    $conex=Conexion::getInstance();
    $sql="SELECT *, DATE_FORMAT(fechaFin, '%d/%m/%Y') AS fechaF, DATE_FORMAT(fecha_creacion, '%d/%m/%Y') AS fechaC FROM ofertas  where id_oferta=$id";
    // Especificamos el fetch mode antes de llamar a fetch()
    $stmt = $conex->dbh->prepare($sql);
    $stmt->setFetchMode(PDO::FETCH_ASSOC);
    // Excecute
    ...
    if (!$row) {
        ...
        $oferta=['id'=> $row['id_oferta'],'descripcion'=>$row['descripcion'],'contacto'=>$row['contacto'],
            ...
            'poblacion'=>$row['poblacion'],'cp'=>$row['cp'],'provincia'=>$row['provincia'], 
            'estado'=>$row['estado'],'fechaInicial'=>$row['fechaC'],'fechaFin'=>$row['fechaF'],
            'psicologo'=>$row['psicologo'],'candidato'=>$row['candidato'],
...
function nuevaOferta( $id,$descripcion, $contacto, $telefono, $email, $direccion, $poblacion,
    $cp, $provincia, $estado, $fecha_creacion, $fechaFin, $psicologo,$candidato, 
    $observaciones) {
  $conex=Conexion::getInstance();
...
function modificarOferta( $descripcion, $contacto, $telefono, $email, $direccion, $poblacion,
    $cp, $provincia, $estado, $fechaFin, $psicologo,$candidato,
    $observaciones, $id_oferta){
        
        $conex=Conexion::getInstance();
       
        $sql=" UPDATE `ofertas` SET `descripcion`='$descripcion',`contacto`='$contacto', `telefono`='$telefono',
        `email`='$email',`direccion`='$direccion',`poblacion`='$poblacion',`cp`='$cp', `provincia`='$provincia',
...
function saveOferta( $datos) {
    ...
    $sql=" UPDATE `ofertas` SET `descripcion`=?,`contacto`=?, `telefono`=?,`email`=?,`direccion`=?,
            `poblacion`=?,`cp`=?, `provincia`=?,`estado`=?, `fechaFin`=?,`psicologo`=?,`candidato`=?,
